#####################
### Gathers data on number of nursing home spaces
### table: https://www.ssb.no/en/statbank/table/11875/
### Time period: 2015-2018
### Municipalities: classification "All municipalities"
###
### Henning Øien, November 2019
#####################
...
# OBS! run 0_Master.R before running this code. It sets directiories and loads packages.
    ...
    url <- 'https://data.ssb.no/api/v0/en/table/11875/'
  ...
  "response": {
    ...
    if(sum(c("KOKkommuneregion0000", "Tid", "contents") %in% names(data)) != 3){
    ...
    var_labels_orig <- unique(data$contents)
    
    var_labels <- var_labels_orig
    
    
    ### Varnames
    
    var_names <- c("knr", "kname", "year", "nh.beds.located.in.muni")
    
    ### Variable_units
    
    value_label <- rep("Number of beds", length(var_names))
    
    ### Time period
    
    time <- sort(as.numeric(unique(data$Tid)))
    
    time_period <- rep("2015 - 2018", length(var_names))
    ...
    source <- rep("ssb.no/en/statbank/table/11875", length(var_names))
    ...
    write_csv(codebook, 
              path = paste(documentation.dir, "4_0_cb_nursing_home_beds_2015_18_t11875.csv", sep = "/"))
    ...
    data <- data[, c("KOKkommuneregion0000", "region", "Tid", "value")]
    
    setnames(data, var_names)
    
    # Save the data
    
    write_csv(data, path = paste(datadir_4, 
                                 "4_0_nursing_home_beds_2015_18_t11875.csv", sep = "/"))
